  ...
  // ── Single-sheet ZIP builder ─────────────────────────────────────────────────
  function buildSingleSheet(sheetXml, sharedStrXml, stylesXml, sheetName) {
  ...
  // ── Public API ────────────────────────────────────────────────────────────────
  function generateIncomeReport(data) {
    const sh = buildIncomeSheet(data);
    return buildSingleSheet(sh.toXml(), makeSharedStrings(sh.strings), makeStyles(), 'Income Report');
  }

  function generateGSTR1Report(data) {
    const sh = buildGSTR1Sheet(data);
    return buildSingleSheet(sh.toXml(), makeSharedStrings(sh.strings), makeStyles(), 'GSTR-1 Report');
  }

  module.exports = { generateIncomeReport, generateGSTR1Report };