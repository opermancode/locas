  ...
  *Total: ${formatINR(invoice.total)}*${invoice.paid > 0 ? `\nPaid: ${formatINR(invoice.paid)}\nBalance: ${formatINR(balance)}` : ''}`;
    ...
    const RightPanel = (
      <View style={s.rightOuter}>
        ...
        {/* ── Top toolbar: actions + template + color all in one compact bar ── */}
        <View style={s.rightToolbar}>

          {/* Action buttons — compact icon+text */}
          <View style={s.rightToolbarActions}>
            <TouchableOpacity
            ...
            >
              {printing
                ? <ActivityIndicator size="small" color="#fff" />
                : <><Icon name="download" size={13} color="#fff" /><Text style={s.rightActPrimaryTxt}> PDF</Text></>
              }
            </TouchableOpacity>
            <TouchableOpacity style={s.rightActSec} onPress={handleWhatsApp}>
              <Icon name="message-circle" size={13} color={COLORS.textSub} />
              <Text style={s.rightActSecTxt}>WhatsApp</Text>
            </TouchableOpacity>
            <TouchableOpacity style={s.rightActSec} onPress={doPDF}>
              <Icon name="printer" size={13} color={COLORS.textSub} />
              <Text style={s.rightActSecTxt}>Print</Text>
            </TouchableOpacity>
          </View>

          {/* Template pills — horizontal scroll, no title */}
          <ScrollView
            horizontal
            showsHorizontalScrollIndicator={false}
            style={s.tplScroll}
            contentContainerStyle={s.tplScrollContent}
          >
            {TEMPLATES.map(tpl => {
          ...
          {/* Color dots — horizontal row, no title, no labels */}
          <ScrollView
            horizontal
            showsHorizontalScrollIndicator={false}
            style={s.colorScroll}
            ...
            {COLOR_PALETTE.map(c => (
              ...
              >
                <View style={[
                  ...
                  {accentColor === c.hex && <Icon name="check" size={10} color="#fff" />}
                </View>
              </TouchableOpacity>
            ))}
          </ScrollView>
  ...
  // ── PreviewPane — paginated A4 preview with zoom controls ────────
  // Injects a postMessage height reporter into the HTML so the iframe
  // auto-expands to show ALL pages. Pages appear as white A4 cards
  // separated by grey gaps — just like a real PDF viewer.
  function PreviewPane({ html, previewZoom, setPreviewZoom }) {
    const containerRef  = React.useRef(null);
    const iframeRef     = React.useRef(null);
    const [containerW,  setContainerW]  = React.useState(0);
    const [docHeight,   setDocHeight]   = React.useState(1123); // default 1 A4 page px

    const A4_W_PX = 794;   // A4 width at 96 dpi
    const A4_H_PX = 1123;  // A4 height at 96 dpi

    // Observe container width for fit-zoom calculation
    React.useEffect(() => {
    ...
    // Listen for height messages from inside the iframe
    React.useEffect(() => {
      const onMsg = (e) => {
        if (e.data?.type === 'locas-doc-height' && typeof e.data.height === 'number') {
          setDocHeight(Math.max(A4_H_PX, e.data.height));
        }
      };
    ...
    // Inject a tiny script into the HTML that reports its full scroll height
    // and adds page-break visual separators between A4 pages
  ...
  (function() {
    function report() {
      ...
      window.parent.postMessage({ type: 'locas-doc-height', height: h }, '*');
    }
  ...
  <\/script>
  <style>
    /* Screen-only: inject A4 page break lines so user sees page boundaries */
    @media screen {
  ...
  <\/style>`;
      ...
      if (html.includes('</head>')) {
        return html.replace('</head>', reporter + '</head>');
      }
      return html + reporter;
    }, [html]);

    const blobUrl = React.useMemo(() => {
      if (!instrumentedHtml) return null;
      const blob = new Blob([instrumentedHtml], { type: 'text/html;charset=utf-8' });
      return URL.createObjectURL(blob);
    }, [instrumentedHtml]);

    React.useEffect(() => () => { if (blobUrl) URL.revokeObjectURL(blobUrl); }, [blobUrl]);

    const fitZoom  = containerW > 0 ? (containerW - 24) / A4_W_PX : 0.7;
    const zoom     = previewZoom !== null ? previewZoom : fitZoom;
    const isFit    = previewZoom === null;

    // How many A4 pages does the content span?
    const pageCount    = Math.ceil(docHeight / A4_H_PX);
    // Total visual height = pages * A4 height + gaps between pages
    const PAGE_GAP     = 12; // px between page cards in the preview
    const scaledH      = (docHeight + (pageCount - 1) * PAGE_GAP) * zoom;

    const zoomIn  = () => setPreviewZoom(z => Math.min(2.0, parseFloat(((z !== null ? z : fitZoom) + 0.1).toFixed(2))));
    const zoomOut = () => setPreviewZoom(z => Math.max(0.25, parseFloat(((z !== null ? z : fitZoom) - 0.1).toFixed(2))));
    ...
    return React.createElement('div', {
      ...
      React.createElement('div', {
        style: {
          ...
          marginBottom: 6, justifyContent: 'flex-end', flexShrink: 0,
        },
      },
        pageCount > 1 && React.createElement('span', {
          style: { fontSize: 10, color: '#94a3b8', marginRight: 6, fontFamily: 'monospace' },
        }, `${pageCount} pages`),
        React.createElement('button', { onClick: zoomOut, style: zoomBtnStyle, title: 'Zoom out' }, '−'),
        React.createElement('button', {
          onClick: resetFit,
          style: {
            ...zoomBtnStyle, minWidth: 48,
            background: isFit ? '#1E40AF' : '#f1f5f9',
            color: isFit ? '#fff' : '#374151',
            fontWeight: 600, fontSize: 11,
            borderColor: isFit ? '#1E40AF' : '#e2e8f0',
          },
          title: 'Reset to fit',
        }, isFit ? 'Fit' : `${Math.round(zoom * 100)}%`),
        React.createElement('button', { onClick: zoomIn, style: zoomBtnStyle, title: 'Zoom in' }, '+'),
      ),
      ...
      React.createElement('div', {
        style: {
          ...
          background: '#e8ecf0',  // grey gutters between pages
          boxSizing: 'border-box',
          padding: `${PAGE_GAP}px`,
        },
      },
        // Scaled wrapper — sets the visual scale
        React.createElement('div', {
          style: {
            width: A4_W_PX * zoom,
            height: scaledH,
          ...
          // The iframe itself renders at full A4 width, scaled down
          React.createElement('iframe', {
            ref: iframeRef,
            src: blobUrl,
            style: {
              border: 'none',
              width: A4_W_PX,
              // iframe intrinsic height = full doc height so all content renders
              height: docHeight,
              display: 'block',
              background: '#fff',
              transformOrigin: 'top left',
              transform: `scale(${zoom})`,
              // Visual page shadow effect
  ...
  const s = StyleSheet.create({
    ...
    // Right panel
    rightOuter:         { flex: 1, display: 'flex', flexDirection: 'column' },
    rightToolbar:       { paddingHorizontal: 10, paddingTop: 8, paddingBottom: 6, borderBottomWidth: 1, borderBottomColor: COLORS.border, backgroundColor: COLORS.card, gap: 6 },
    rightToolbarActions:{ flexDirection: 'row', gap: 6 },
    rightActPrimary:    { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 4, paddingVertical: 7, paddingHorizontal: 12, borderRadius: RADIUS.md },
    rightActPrimaryTxt: { fontSize: 12, fontWeight: FONTS.bold, color: '#fff' },
    rightActSec:        { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 4, paddingVertical: 7, paddingHorizontal: 10, borderRadius: RADIUS.md, backgroundColor: COLORS.bg, borderWidth: 1, borderColor: COLORS.border },
    rightActSecTxt:     { fontSize: 12, fontWeight: FONTS.semibold, color: COLORS.textSub },

    // Template scroll
    tplScroll:        { flexGrow: 0 },
    tplScrollContent: { gap: 6, paddingRight: 4 },

    // Color scroll
    colorScroll:        { flexGrow: 0 },
    colorScrollContent: { gap: 6, paddingRight: 4, alignItems: 'center' },
    colorDotWrap:       { padding: 2 },

    // Preview flex area
    previewFlex: { flex: 1, overflow: 'hidden' },

    // Template pills
    tplPill:    { paddingHorizontal: 11, paddingVertical: 6, borderRadius: RADIUS.full, borderWidth: 1.5, borderColor: COLORS.border, backgroundColor: COLORS.card },
    tplPillTxt: { fontSize: 11, fontWeight: FONTS.semibold, color: COLORS.textSub },

    // Color dots
    colorDot:    { width: 22, height: 22, borderRadius: 11, alignItems: 'center', justifyContent: 'center', borderWidth: 2, borderColor: 'transparent' },
    colorDotSel: { borderColor: COLORS.text, transform: [{ scale: 1.18 }] },
