...
function PreviewPane({ html, previewZoom, setPreviewZoom }) {
  ...
  const [containerH, setContainerH] = React.useState(500);
  const A4_W = 794;
  ...
  const CONTAINER_H = Math.max(containerH, 200);
  ...
  return React.createElement('div', { ref: containerRef, style: { width: '100%', height: '100%', display: 'flex', flexDirection: 'column' } },
    // Zoom toolbar
    React.createElement('div', {
      style: {
      ...
      // Percentage / Fit label — clickable to reset fit
      React.createElement('button', {
        onClick: resetFit,
        style: {
          ...zoomBtnStyle,
    ...
    // Preview container — flex fills remaining height, overflow scroll if zoomed in
    React.createElement('div', {
      style: {
        flex: 1,
        overflow: 'auto',
