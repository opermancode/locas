  ...
  export default function ReportsScreen({ navigation }) {
    ...
    const exportGSTR1CSV = () => {
      ...
      // ── B2B Invoices ──────────────────────────────────────────────
      lines.push('# TABLE 4A — B2B Invoices (with GSTIN)');
      lines.push('Invoice No,Date,Party Name,GSTIN,Place of Supply,Taxable Value,IGST,CGST,SGST,Total Tax,Invoice Value');
      const b2b = sales.filter(i => i.party_gstin);
      b2b.forEach(i => lines.push([
        i.invoice_number, i.date, esc(i.party_name), i.party_gstin,
        esc(i.party_state || ''),
        n2(i.taxable), n2(i.igst||0), n2(i.cgst||0), n2(i.sgst||0),
        n2(i.total_tax||0), n2(i.total),
      ].join(',')));
      lines.push(`B2B Total,,,,, ${n2(b2b.reduce((s,i)=>s+(i.taxable||0),0))},${n2(b2b.reduce((s,i)=>s+(i.igst||0),0))},${n2(b2b.reduce((s,i)=>s+(i.cgst||0),0))},${n2(b2b.reduce((s,i)=>s+(i.sgst||0),0))},${n2(b2b.reduce((s,i)=>s+(i.total_tax||0),0))},${n2(b2b.reduce((s,i)=>s+(i.total||0),0))}`);
      lines.push('');

      // ── B2C Invoices ──────────────────────────────────────────────
      lines.push('# TABLE 7 — B2C / Walk-in (without GSTIN)');
      ...
      lines.push(`B2C Total,,,${n2(b2c.reduce((s,i)=>s+(i.taxable||0),0))},${n2(b2c.reduce((s,i)=>s+(i.igst||0),0))},${n2(b2c.reduce((s,i)=>s+(i.cgst||0),0))},${n2(b2c.reduce((s,i)=>s+(i.sgst||0),0))},${n2(b2c.reduce((s,i)=>s+(i.total_tax||0),0))},${n2(b2c.reduce((s,i)=>s+(i.total||0),0))}`);
      lines.push('');

      // ── HSN Summary (Table 12) ────────────────────────────────────
      lines.push('# TABLE 12 — HSN-wise Summary');
      lines.push('HSN Code,Description,UOM,Total Qty,Taxable Value,IGST,CGST,SGST,Total Tax');

      // Group line items by HSN code
      const hsnMap = {};
      saleLineItems.forEach(li => {
        ...
        entry.qty      += (li.qty      || 0);
        entry.taxable  += (li.taxable  || 0);
        entry.igst     += (li.igst     || 0);
        entry.cgst     += (li.cgst     || 0);
        entry.sgst     += (li.sgst     || 0);
        entry.total_tax+= (li.total_tax|| (li.igst||0)+(li.cgst||0)+(li.sgst||0));
      });

      const hsnRows = Object.values(hsnMap).sort((a, b) => a.hsn.localeCompare(b.hsn));
      hsnRows.forEach(h => lines.push([
        h.hsn, esc(h.description), h.unit,
        h.qty.toFixed(2),
        n2(h.taxable), n2(h.igst), n2(h.cgst), n2(h.sgst), n2(h.total_tax),
      ].join(',')));

      if (hsnRows.length > 0) {
        lines.push([
          'TOTAL','','',
          hsnRows.reduce((s,h)=>s+h.qty,0).toFixed(2),
          n2(hsnRows.reduce((s,h)=>s+h.taxable,0)),
          n2(hsnRows.reduce((s,h)=>s+h.igst,0)),
          n2(hsnRows.reduce((s,h)=>s+h.cgst,0)),
          n2(hsnRows.reduce((s,h)=>s+h.sgst,0)),
          n2(hsnRows.reduce((s,h)=>s+h.total_tax,0)),
        ].join(','));
      }
      lines.push('');
      ...
      lines.push(`Total Taxable Value,${n2(sales.reduce((s,i)=>s+(i.taxable||0),0))}`);
      ...
      lines.push(`B2B Invoices,${b2b.length}`);
      lines.push(`B2C Invoices,${b2c.length}`);
