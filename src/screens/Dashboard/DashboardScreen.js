...
// ── PO Widget ─────────────────────────────────────────────────────
function POWidget({ poStats, selectedMonth, onMonthChange, onNavigate, onPoPress, onNavigatePOAlerts, onNavigateBacklog, pw, s }) {
  const now = new Date();
...
export default function DashboardScreen({ navigation }) {
  ...
  const computePOStats = useCallback((allPOs, { year, month }) => {
    ...
    // remainingValue: only from month POs that are still active/partial
