    ...
    // ── Day/Night theme — 6am-6pm = light, otherwise dark ─────────────
    const _h    = new Date().getHours();
    const _day  = _h >= 6 && _h < 18;
    const DARK    = _day ? '#F1F5F9' : '#0B1120';
    const CARD_BG = _day ? '#FFFFFF' : '#131C2E';
    const _border = _day ? 'rgba(0,0,0,0.08)'  : 'rgba(255,255,255,0.07)';
    const _text   = _day ? '#0F172A'            : '#FFFFFF';
    const _textS  = _day ? '#475569'            : 'rgba(255,255,255,0.7)';
    const _textM  = _day ? 'rgba(0,0,0,0.35)'  : 'rgba(255,255,255,0.3)';
    const _textM2 = _day ? 'rgba(0,0,0,0.22)'  : 'rgba(255,255,255,0.2)';
    const _inner  = _day ? 'rgba(0,0,0,0.04)'  : 'rgba(255,255,255,0.04)';
    const _sbStyle= _day ? 'dark'               : 'light';
    ...
    function AreaChart({ data = [] }) {
            ...
            {data.map((d, i) => {
              ...
              return (
                <View key={i} style={{ flex:1, alignItems:'center', gap:4 }}>
                  ...
                  <Text style={{ fontSize:7, color:_textM }}>{d.label}</Text>
                </View>
              );
            })}
          </View>
      ...
      const glColor = _day ? 'rgba(0,0,0,0.06)' : 'rgba(255,255,255,0.06)';
      const glText  = _day ? 'rgba(0,0,0,0.3)'  : 'rgba(255,255,255,0.25)';
      const ptText  = _day ? 'rgba(0,0,0,0.3)'  : 'rgba(255,255,255,0.28)';
      const gridLines = [0, 0.5, 1].map(pct => {
        const y = (PT + (1-pct)*cH).toFixed(1);
        const lbl = fmt(max * pct);
        return `<line x1="${PL}" y1="${y}" x2="${W-PR}" y2="${y}" stroke="${glColor}" stroke-dasharray="3,4"/>
                <text x="${PL-4}" y="${parseFloat(y)+3}" text-anchor="end" font-size="8" fill="${glText}" font-family="system-ui">${lbl}</text>`;
      }).join('');
      const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="100%" height="${H}" viewBox="0 0 ${W} ${H}">
        ...
        ${pts.map(p=>`<text x="${p.x}" y="${H-2}" text-anchor="middle" font-size="8.5" fill="${ptText}" font-family="system-ui">${p.label}</text>`).join('')}
      </svg>`;
    ...
    function DonutChart({ collected=0, outstanding=0, expenses=0, size=68 }) {
      ...
      if (Platform.OS !== 'web') {
        return <View style={{ width:size, height:size, borderRadius:size/2, backgroundColor:_inner }} />;
      }
      ...
      const svg = `<svg width="${size}" height="${size}" viewBox="0 0 ${size} ${size}"><circle cx="${cx}" cy="${cy}" r="${r}" fill="none" stroke="${_inner}" stroke-width="${stroke}"/>${paths.join('')}</svg>`;
      ...
      return (
        <View style={pw.card}>
          ...
          <View style={pw.monthRow}>
            <TouchableOpacity onPress={prevMonth} style={pw.monthArrow}><Icon name="chevron-left" size={13} color={_textM}/></TouchableOpacity>
            <Text style={pw.monthTxt}>{MONTH_NAMES[selectedMonth.month]} {selectedMonth.year}{isCurrent?' · Now':''}</Text>
            <TouchableOpacity onPress={nextMonth} style={[pw.monthArrow,isCurrent&&{opacity:0.25}]} disabled={isCurrent}><Icon name="chevron-right" size={13} color={_textM}/></TouchableOpacity>
          </View>

          {/* Stats */}
          <View style={pw.statsRow}>
            {[
              {val:String(poStats?.count??'—'), lbl:'Orders', color:_text, onPress:null},
              {val:poStats?.total?formatINRCompact(poStats.total):'₹—', lbl:'Total Value', color:'#4ADE80', onPress:onNavigate},
              {val:poStats?.remainingValue?formatINRCompact(poStats.remainingValue):'₹0', lbl:'Unfulfilled', color:'#F87171', onPress:()=>poStats?.remainingValue>0&&onNavigateBacklog&&onNavigateBacklog()},
          ...
          {poStats&&poStats.count===0&&(
            <View style={{paddingVertical:14,alignItems:'center'}}>
              <Text style={{fontSize:11,color:_textM}}>No POs this month</Text>
            </View>
          )}
        </View>
      );
    ...
    export default function DashboardScreen({ navigation }) {
      ...
      const computePOStats = useCallback((allPOs, { year, month }) => {
        ...
        const remainingValue = monthPOs
          .filter(po=>po.status==='active'||po.status==='partial')
          .reduce((s,po)=>s+(po.remainingValue||0),0);
      ...
      const QUICK = [
        ...
        { label:'Settings',  icon:'settings',     color:_textM,    bg:_inner,                   go:()=>navigation.navigate('More',{ screen:'Settings' }) },
      ];
              ...
              <View style={s.leftCol}>
                ...
                <View style={{flexDirection:'row',gap:7,flex:1}}>
                  <View style={[s.listCard,{flex:1}]}>
                    ...
                    {topParties.length===0?(
                      <Text style={{fontSize:11,color:_textM2,paddingVertical:8}}>No parties yet</Text>
                    ):topParties.map((p,i)=>{
                      ...
                      return (
                        <TouchableOpacity key={p.id} style={[s.row,i<topParties.length-1&&s.rowBorder]} onPress={()=>navigation.navigate('PartiesTab',{screen:'PartyDetail',params:{partyId:p.id}})} activeOpacity={0.75}>
                          ...
                          <View style={{flex:1}}>
                            ...
                            <Text style={[s.rowAmtSub,{color:bal===0?_textM2:isCredit?'#4ADE80':'#F87171'}]}>{bal===0?'Settled':isCredit?'+'+formatINRCompact(bal):'-'+formatINRCompact(Math.abs(bal))}</Text>
                          </View>
                        </TouchableOpacity>
                      );
                    })}
                  ...
                  <View style={[s.listCard,{flex:1}]}>
                    ...
                    {recentInvoices.length===0?(
                      ...
                      const sc=isPaid?{color:'#4ADE80',label:'Paid'}:isOD?{color:'#F87171',label:'Overdue'}:inv.status==='partial'?{color:'#FCD34D',label:'Partial'}:{color:_textM,label:'Unpaid'};
                      return (
              ...
              <View style={s.rightCol}>
                ...
                <View style={s.alertStrip}>
                  <TouchableOpacity style={[s.alertChip, overdueInvCount>0 && s.alertChipRed]} onPress={()=>navigation.navigate('InvoicesTab',{screen:'InvoiceList'})} activeOpacity={0.8}>
                    <Icon name="alert-circle" size={12} color={overdueInvCount>0?'#F87171':_textM}/>
                  ...
                  <TouchableOpacity style={[s.alertChip, lowStockCount>0 && s.alertChipYellow]} onPress={()=>navigation.navigate('Inventory')} activeOpacity={0.8}>
                    <Icon name="alert-triangle" size={12} color={lowStockCount>0?'#FCD34D':_textM}/>
                    ...
                    <View style={{flex:1,gap:6}}>
                      {[
                        ...
                        <View key={i} style={s.legendRow}>
                          ...
                          <Text style={{fontSize:8,color:_textM2,marginLeft:3}}>{it.pct}%</Text>
                        </View>
                      ))}
                    </View>
                  </View>
                ...
                <View style={[s.listCard,{flex:1}]}>
                  ...
                  {recentQuotes.length===0?(
                    ...
                    const sc = q.status==='converted'?{color:'#4ADE80',label:'Converted'}
                            : q.status==='accepted' ?{color:'#34D399',label:'Accepted'}
                            : q.status==='expired'  ?{color:'#F87171',label:'Expired'}
                            : {color:'#FCD34D',label:'Pending'};
                    return (
                      <TouchableOpacity key={q.id} style={[s.row,i<recentQuotes.length-1&&s.rowBorder]} onPress={()=>navigation.navigate('QuotationsTab',{screen:'QuotationDetail',params:{id:q.id}})} activeOpacity={0.75}>
                        <View style={{flex:1}}>
              ...
              <View style={[s.searchSheet,{paddingTop:insets.top+8}]}>
                <View style={s.searchHeader}>
                  <View style={s.searchInputWrap}>
                    <Icon name="search" size={15} color={_textM}/>
                    <TextInput
                      ref={searchRef}
                      style={s.searchInput}
                      placeholder="Search invoices, parties, products..."
                      placeholderTextColor={_textM}
                      value={searchQuery}
                    ...
                    {searchQuery.length>0&&(
                      <TouchableOpacity onPress={()=>handleSearch('')}>
                        <Icon name="x" size={14} color={_textM}/>
                      </TouchableOpacity>
                    )}
                  </View>
                ...
                <ScrollView keyboardShouldPersistTaps="handled" style={{flex:1}}>
                  {searching?(
                    <View style={{flexDirection:'row',alignItems:'center',justifyContent:'center',paddingVertical:40,gap:10}}>
                      ...
                      <Text style={{fontSize:14,color:_textM}}>Searching...</Text>
                    </View>
                  ):searchResults?(()=>{
                    ...
                    return total===0?(
                      <View style={{alignItems:'center',paddingVertical:48,gap:10}}>
                        <Icon name="search" size={36} color={_textM}/>
                        <Text style={{fontSize:14,color:_textM}}>No results for "{searchQuery}"</Text>
                      </View>
                    ):(
                      <>
                        {[
                          ...
                          searchResults[key]?.length>0?(
                            <View key={key} style={{padding:14}}>
                              <Text style={{fontSize:10,fontWeight:'700',color:_textM,textTransform:'uppercase',letterSpacing:0.7,marginBottom:8}}>{label} ({searchResults[key].length})</Text>
                              {searchResults[key].map(item=>{
                                ...
                                return (
                                  <TouchableOpacity key={`${type}-${item.id}`} style={s.searchResult} onPress={()=>goResult(type,item)}>
                                    ...
                                    <Icon name="chevron-right" size={13} color={_textM2}/>
                                  </TouchableOpacity>
                                );
                              })}
                            </View>
                    ...
                    <View style={{padding:20}}>
                      <Text style={{fontSize:15,fontWeight:'700',color:_textS,marginBottom:16}}>Search across everything</Text>
                      {[
                        ...
                        <View key={i} style={{flexDirection:'row',alignItems:'center',gap:12,paddingVertical:10}}>
                          ...
                          <Text style={{fontSize:13,color:_textM}}>{h.text}</Text>
                        </View>
                      ))}
                    </View>
                  )}
    ...
    const s = StyleSheet.create({
      ...
      topBar:    { flexDirection:'row', alignItems:'center', justifyContent:'space-between', backgroundColor:CARD_BG, paddingHorizontal:14, paddingVertical:8, borderBottomWidth:1, borderBottomColor:_border },
      topLeft:   { flex:1 },
      greetTxt:  { fontSize:10, color:_textM, marginBottom:1, letterSpacing:0.2 },
      bizName:   { fontSize:15, fontWeight:'800', color:_text },
      topRight:  { flexDirection:'row', gap:6 },
      topBtn:    { width:30, height:30, borderRadius:8, backgroundColor:_inner, alignItems:'center', justifyContent:'center', borderWidth:1, borderColor:_border },
      ...
      heroCard:   { backgroundColor:CARD_BG, borderRadius:14, padding:12, borderWidth:1, borderColor:_border },
      heroHeader: { flexDirection:'row', alignItems:'flex-start', marginBottom:10 },
      heroEye:    { fontSize:8, color:_textM, letterSpacing:1.5, marginBottom:3 },
      heroAmt:    { fontSize:22, fontWeight:'900', color:_text, marginBottom:1 },
      heroSub:    { fontSize:9, color:_textM },
      ...
      heroStats:  { flexDirection:'row', backgroundColor:_inner, borderRadius:8, padding:8 },
      heroStat:   { flex:1, alignItems:'center' },
      heroStatVal:{ fontSize:10, fontWeight:'800', marginBottom:1 },
      heroStatLbl:{ fontSize:7, color:_textM, textTransform:'uppercase', letterSpacing:0.4 },
      heroDivider:{ width:1, backgroundColor:_border },

      chartCard: { backgroundColor:CARD_BG, borderRadius:14, padding:10, borderWidth:1, borderColor:_border },
      chartHead: { flexDirection:'row', alignItems:'center', justifyContent:'space-between', marginBottom:4 },
      chartTitle:{ fontSize:12, fontWeight:'700', color:_textS },
      chartSub:  { fontSize:9, color:_textM, textTransform:'uppercase', letterSpacing:0.5 },

      qaCard:  { backgroundColor:CARD_BG, borderRadius:14, padding:10, borderWidth:1, borderColor:_border },
      qaRow:   { flexDirection:'row', justifyContent:'space-between' },
      qaItem:  { flex:1, alignItems:'center', gap:4 },
      qaIcon:  { width:34, height:34, borderRadius:10, alignItems:'center', justifyContent:'center', borderWidth:1, borderColor:_border },
      qaLabel: { fontSize:7.5, color:_textM, textAlign:'center', fontWeight:'500' },

      kpiGrid: { flexDirection:'row', flexWrap:'wrap', gap:6 },
      kpiTile: { width:'47.5%', backgroundColor:CARD_BG, borderRadius:12, padding:10, borderWidth:1, borderColor:_border, gap:2 },
      ...
      kpiLbl:  { fontSize:10, fontWeight:'600', color:_textS },
      kpiSub:  { fontSize:8, color:_textM },
      alertStrip:     { flexDirection:'row', gap:6 },
      alertChip:      { flex:1, flexDirection:'row', alignItems:'center', gap:5, backgroundColor:_inner, borderRadius:8, paddingVertical:7, paddingHorizontal:9, borderWidth:1, borderColor:_border },
      ...
      alertChipTxt:   { fontSize:10, fontWeight:'600', color:_textM, flex:1 },

      donutCard:  { backgroundColor:CARD_BG, borderRadius:14, padding:10, borderWidth:1, borderColor:_border },
      donutTitle: { fontSize:9, fontWeight:'700', color:_textM, textTransform:'uppercase', letterSpacing:0.8, marginBottom:8 },
      donutBody:  { flexDirection:'row', alignItems:'center', gap:10 },
      legendRow:  { flexDirection:'row', alignItems:'center', gap:6 },
      legendDot:  { width:6, height:6, borderRadius:3 },
      legendLbl:  { flex:1, fontSize:10, color:_textM },
      legendVal:  { fontSize:10, fontWeight:'700', color:_textS },

      listCard:{ flex:1, backgroundColor:CARD_BG, borderRadius:14, padding:10, borderWidth:1, borderColor:_border },
      listHead:{ flexDirection:'row', alignItems:'center', justifyContent:'space-between', marginBottom:8 },
      listTitle:{ fontSize:9, fontWeight:'700', color:_textM, textTransform:'uppercase', letterSpacing:1 },
      listAction:{ fontSize:10, color:BRAND, fontWeight:'600' },
      row:      { flexDirection:'row', alignItems:'center', paddingVertical:6, gap:8 },
      rowBorder:{ borderBottomWidth:1, borderBottomColor:_border },
      rowName:  { fontSize:11, fontWeight:'700', color:_text },
      rowSub:   { fontSize:9, color:_textM, marginTop:1 },
      rowAmt:   { fontSize:11, fontWeight:'800', color:_textS },
      rowAmtSub:{ fontSize:9, color:_textM },
      ...
      searchOverlay:    { flex:1, backgroundColor:_day?'rgba(0,0,0,0.4)':'rgba(0,0,0,0.7)' },
      searchSheet:      { flex:1, backgroundColor:DARK },
      searchHeader:     { flexDirection:'row', alignItems:'center', gap:10, paddingHorizontal:14, paddingBottom:10, backgroundColor:CARD_BG, borderBottomWidth:1, borderBottomColor:_border },
      searchInputWrap:  { flex:1, flexDirection:'row', alignItems:'center', gap:8, backgroundColor:_inner, borderRadius:10, paddingHorizontal:12, paddingVertical:9, borderWidth:1, borderColor:_border },
      searchInput:      { flex:1, fontSize:14, color:_text, paddingVertical:0 },
      searchCancel:     { paddingLeft:4, paddingVertical:8 },
      searchCancelTxt:  { fontSize:14, fontWeight:'600', color:BRAND },
      searchResult:     { flexDirection:'row', alignItems:'center', gap:10, backgroundColor:CARD_BG, borderRadius:12, padding:11, marginBottom:6, borderWidth:1, borderColor:_border },
      searchResultIcon: { width:34, height:34, borderRadius:9, alignItems:'center', justifyContent:'center', flexShrink:0 },
      searchResultTitle:{ fontSize:13, fontWeight:'600', color:_text },
      searchResultSub:  { fontSize:11, color:_textM, marginTop:1 },
    });

    // ── PO Widget Styles ───────────────────────────────────────────────
    const pw = StyleSheet.create({
      card:       { backgroundColor:CARD_BG, borderRadius:14, padding:10, borderWidth:1, borderColor:_border },
      ...
      title:      { fontSize:10, fontWeight:'700', color:_textM, textTransform:'uppercase', letterSpacing:1 },
      ...
      monthRow:   { flexDirection:'row', alignItems:'center', justifyContent:'space-between', backgroundColor:_inner, borderRadius:10, paddingVertical:7, paddingHorizontal:10, marginBottom:10 },
      monthArrow: { padding:3 },
      monthTxt:   { fontSize:12, fontWeight:'600', color:_textS },

      statsRow:   { flexDirection:'row', backgroundColor:_inner, borderRadius:10, padding:10, marginBottom:10 },
      statCell:   { flex:1, alignItems:'center' },
      statDiv:    { width:1, backgroundColor:_border, marginVertical:2 },
      statVal:    { fontSize:14, fontWeight:'900', marginBottom:2 },
      statLbl:    { fontSize:8, color:_textM, textTransform:'uppercase', letterSpacing:0.4 },
      ...
      poRowBorder:{ borderBottomWidth:1, borderBottomColor:_border },
      poAccent:   { width:3, height:32, borderRadius:2, flexShrink:0 },
      poNum:      { fontSize:12, fontWeight:'700', color:_text },
      poParty:    { fontSize:10, color:_textM, marginTop:1 },
