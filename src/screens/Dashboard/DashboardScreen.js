import React, { useState, useCallback, useEffect, useRef } from 'react';
  ...
  import { COLORS as COLORS_BASE, RADIUS, FONTS } from '../../theme';

  // Day (6am-6pm) = light, Night = dark — set once at module load, no re-renders
  const _h = new Date().getHours();
  const COLORS = (_h >= 6 && _h < 18) ? COLORS_BASE : {
    ...COLORS_BASE,
    bg:        '#0B1120',
    bgDeep:    '#0F1825',
    card:      '#131C2E',
  ...
  export default function DashboardScreen({ navigation }) {
    ...
    return (
      <View style={[s.container, { paddingTop: insets.top }]}>
        <StatusBar style={(_h >= 6 && _h < 18) ? 'dark' : 'light'} />
        <Animated.View style={[s.fill, { opacity: fadeAnim }]}>
          ...
          <View style={s.grid}>
            ...
            <View style={s.leftCol}>
              ...
              {/* ── Top Parties — fills empty space below Quick Actions ── */}
              {topParties.length > 0 && (
