...
export default function InventoryScreen({ navigation, route }) {
  ...
  const openEdit = (item) => {
    setForm({
      ...
      unit: item.unit || 'pcs', hsn: item.hsn || '',
  ...
  const handleDownloadTemplate = () => {
    downloadCSV('LOCAS_Items_Template.csv', [ITEM_TEMPLATE_COLS, ...ITEM_TEMPLATE_SAMPLE]);
  };
  ...
  const handlePickCSV = () => {
    ...
    input.onchange = async () => {
      ...
      setBulkLoading(true);
      try {
        const results = parseItemsCSV(await file.text());
        if (!results.length) { window.alert('No valid items found in file.'); return; }
        setBulkParsed(results); setBulkDone(null);
      } catch (e) {
  ...
  const exportStockInventory = () => {
    if (items.length === 0) {
      if (Platform.OS === 'web') { window.alert('No items to export.'); }
      else { Alert.alert('No Items', 'Add some inventory items first.'); }
      return;
    }
    try {
      const now = new Date();
      const dateStr = now.toLocaleDateString('en-IN', { day: '2-digit', month: '2-digit', year: 'numeric' }).replace(/\//g, '-');
      const headers = ['Sr No', 'Item Name', 'Code', 'HSN/SAC', 'Unit', 'Sale Price (₹)', 'Cost Price (₹)', 'GST Rate (%)', 'Current Stock', 'Min Stock', 'Stock Value (₹)', 'Status'];
      const rows = items.map((item, idx) => {
        ...
        const esc = (val) => { const str = String(val || ''); return str.includes(',') || str.includes('"') || str.includes('\n') ? `"${str.replace(/"/g, '""')}"` : str; };
        return [idx + 1, esc(item.name), esc(item.code || ''), esc(item.hsn || ''), esc(item.unit || 'pcs'), (item.sale_price || 0).toFixed(2), (item.purchase_price || 0).toFixed(2), item.gst_rate || 0, item.stock || 0, item.min_stock || 0, stockValue.toFixed(2), status].join(',');
      });
      const totalValue = items.reduce((s, i) => s + ((i.stock || 0) * (i.sale_price || 0)), 0);
      const csv = [`LOCAS — Stock Inventory Report`, `Generated: ${dateStr}`, '', headers.join(','), ...rows, '', `,,,,,,,,,,Total Stock Value,${totalValue.toFixed(2)}`].join('\n');
      if (Platform.OS === 'web') {
        const blob = new Blob(['\uFEFF' + csv], { type: 'text/csv;charset=utf-8;' });
        const url  = URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url; link.download = `Stock_Inventory_${dateStr}.csv`;
        document.body.appendChild(link); link.click(); document.body.removeChild(link);
        URL.revokeObjectURL(url);
      } else { Alert.alert('Export', 'CSV export is available on desktop/web version.'); }
    ...
    <View style={[s.container, { paddingTop: insets.top }]}>
      ...
      <View style={s.header}>
        ...
        <View style={s.headerBtns}>
          <TouchableOpacity style={s.excelBtn} onPress={exportStockInventory}>
            <ExcelIcon /><Text style={s.excelBtnTxt}>Export</Text>
          </TouchableOpacity>
      ...
      <View style={s.toolbar}>
        <View style={s.searchBox}>
          ...
          <TextInput style={s.searchInput} placeholder="Search name, code, HSN..." placeholderTextColor={COLORS.textMute} value={search} onChangeText={handleSearch} />
          {search.length > 0 && (<TouchableOpacity onPress={() => handleSearch('')}><Icon name="x" size={14} color={COLORS.textMute} /></TouchableOpacity>)}
        </View>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={s.chipScroll} contentContainerStyle={s.chipRow}>
          {[
            ...
            <TouchableOpacity key={f.key} style={[s.chip, typeFilter === f.key && s.chipActive]} onPress={() => handleTypeFilter(f.key)}>
              <Text style={[s.chipTxt, typeFilter === f.key && s.chipTxtActive]}>{f.label}</Text>
            </TouchableOpacity>
          ))}
          {lowStockCount > 0 && (
            <TouchableOpacity style={[s.chip, lowStockOnly && s.chipDanger]} onPress={toggleLow}>
              <Text style={[s.chipTxt, lowStockOnly && s.chipTxtDanger]}>⚠ Low Stock ({lowStockCount})</Text>
            </TouchableOpacity>
          )}
        </ScrollView>
      </View>
      ...
      <View style={s.tableWrap}>
        ...
        <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={COLORS.primary} />} showsVerticalScrollIndicator={false}>
          {sorted.length === 0 ? (
            <View style={s.empty}>
              <View style={s.emptyIcon}><Icon name="package" size={28} color={COLORS.primary} /></View>
              <Text style={s.emptyTitle}>{search || typeFilter !== 'all' || lowStockOnly ? 'No items match' : 'No items yet'}</Text>
              <Text style={s.emptySub}>{search || typeFilter !== 'all' ? 'Try a different search or filter' : 'Add products and services to your inventory'}</Text>
              {!search && typeFilter === 'all' && !lowStockOnly && (
                <TouchableOpacity style={s.emptyBtn} onPress={openAdd}><Text style={s.emptyBtnTxt}>+ Add First Item</Text></TouchableOpacity>
              )}
            </View>
          ) : (
            sorted.map((item, idx) => {
              ...
              return (
                <View key={item.id} style={[s.trow, { backgroundColor: rowBg }, isLow && s.trowLow, isNeg && s.trowNeg]}>
                  ...
                  <View style={{ width: 72, alignItems: 'center', justifyContent: 'center' }}>
                    <View style={[s.typePill, isService ? s.typePillSvc : s.typePillProd]}>
                      <Text style={[s.typePillTxt, isService ? s.typePillSvcTxt : s.typePillProdTxt]}>{isService ? 'Service' : 'Product'}</Text>
                    </View>
                  </View>
                  <Text style={[s.td, { width: 78 }]} numberOfLines={1}>{item.hsn || '—'}</Text>
                  <Text style={[s.td, { width: 54, textAlign: 'center' }]}>{item.unit}</Text>
                  <Text style={[s.td, { flex: 1, textAlign: 'right', fontWeight: FONTS.bold }]}>{formatINR(item.sale_price)}</Text>
                  <Text style={[s.td, { flex: 1, textAlign: 'right', color: COLORS.textSub }]}>{item.purchase_price > 0 ? formatINR(item.purchase_price) : '—'}</Text>
                  ...
                  <View style={{ flex: 0.9, alignItems: 'flex-end', justifyContent: 'center', paddingHorizontal: 8 }}>
                    {isService
                      ...
                      : <Text style={[s.td, { fontWeight: FONTS.semibold }, isNeg && { color: COLORS.danger }, isLow && { color: COLORS.warning }]}>{item.stock}</Text>
                    }
                    {isLow && <Text style={s.lowHint}>Low</Text>}
                    {isNeg && <Text style={s.negHint}>Neg</Text>}
                  </View>
                  <Text style={[s.td, { width: 50, textAlign: 'center', color: COLORS.textMute }]}>{isService ? '—' : (item.min_stock || '—')}</Text>
                  <Text style={[s.td, { flex: 1.1, textAlign: 'right' }, isNeg && { color: COLORS.danger }]}>{isService ? '—' : formatINR(stockValue)}</Text>
                  <View style={s.tdActions}>
                    <TouchableOpacity style={s.editBtn} onPress={() => openEdit(item)}><Icon name="edit-2" size={13} color={COLORS.textSub} /></TouchableOpacity>
                    <TouchableOpacity style={s.delBtn}  onPress={() => handleDelete(item)}><Icon name="trash-2" size={13} color={COLORS.danger} /></TouchableOpacity>
                  </View>
                </View>
              );
            })
        ...
        <View style={s.modalContainer}>
          ...
          <ScrollView style={s.modalScroll} keyboardShouldPersistTaps="handled">
                ...
                {ITEM_TEMPLATE_COLS.map((col, i) => (
                  <View key={i} style={[s.templateCol, col.includes('*') && s.templateColRequired]}>
                    <Text style={[s.templateColTxt, col.includes('*') && s.templateColTxtRequired]}>
                      {col.replace('*', '')}{col.includes('*') ? <Text style={{ color: COLORS.danger }}>*</Text> : null}
                    </Text>
                  </View>
                ))}
              </View>
            ...
            <TouchableOpacity style={s.downloadBtn} onPress={handleDownloadTemplate}>
              <ExcelIcon /><Text style={s.downloadBtnTxt}>Download Template (.csv)</Text>
            </TouchableOpacity>
            ...
            {!bulkParsed && !bulkDone && (
              <TouchableOpacity style={s.uploadZone} onPress={handlePickCSV} disabled={bulkLoading}>
                {bulkLoading
                  ...
                  : <>
                      <View style={s.uploadZoneIcon}><Icon name="upload" size={28} color={COLORS.primary} /></View>
      ...
      <Modal visible={modal} animationType="slide" presentationStyle="pageSheet">
        <View style={s.modalContainer}>
          <View style={s.modalHeader}>
            <TouchableOpacity onPress={() => setModal(false)}><Text style={s.modalCancel}>Cancel</Text></TouchableOpacity>
          ...
          <View style={s.typeToggleBar}>
            <TouchableOpacity style={[s.typeBtn, form.item_type === 'product' && s.typeBtnActive]} onPress={() => setForm(f => ({ ...f, item_type: 'product', unit: 'pcs' }))}>
              <Icon name="package" size={15} color={form.item_type === 'product' ? '#fff' : COLORS.textSub} />
              <Text style={[s.typeBtnTxt, form.item_type === 'product' && s.typeBtnTxtActive]}>Product</Text>
            </TouchableOpacity>
            <TouchableOpacity style={[s.typeBtn, form.item_type === 'service' && s.typeBtnServiceActive]} onPress={() => setForm(f => ({ ...f, item_type: 'service', unit: 'hrs', stock: '0', min_stock: '0' }))}>
              <Icon name="tool" size={15} color={form.item_type === 'service' ? '#fff' : COLORS.textSub} />
              <Text style={[s.typeBtnTxt, form.item_type === 'service' && s.typeBtnTxtActive]}>Service</Text>
            </TouchableOpacity>
          </View>
          <View style={[s.typeHint, form.item_type === 'service' ? s.typeHintService : s.typeHintProduct]}>
            <Icon name={form.item_type === 'service' ? 'tool' : 'package'} size={12} color={form.item_type === 'service' ? '#0EA5E9' : COLORS.primary} />
            <Text style={[s.typeHintTxt, form.item_type === 'service' ? { color: '#0369A1' } : { color: COLORS.primary }]}>
              {form.item_type === 'service' ? 'Services have no stock tracking — ideal for labour, consulting, repairs, delivery etc.' : 'Products track stock levels and trigger low-stock alerts.'}
            </Text>
          </View>
          <ScrollView style={s.modalScroll} keyboardShouldPersistTaps="handled">
            <FieldLabel>{form.item_type === 'service' ? 'Service Name *' : 'Product Name *'}</FieldLabel>
            <TextInput style={s.input} value={form.name} onChangeText={v => setForm(f => ({ ...f, name: v }))} placeholder={form.item_type === 'service' ? 'e.g. Installation Charges' : 'e.g. Rice Bag 25kg'} placeholderTextColor={COLORS.textMute} />
            {form.item_type === 'service' && (
              <>
                <FieldLabel>Description (optional)</FieldLabel>
                <TextInput style={[s.input, { minHeight: 60, textAlignVertical: 'top' }]} value={form.description} onChangeText={v => setForm(f => ({ ...f, description: v }))} placeholder="Brief description of the service..." placeholderTextColor={COLORS.textMute} multiline />
              </>
            )}
            <View style={s.row}>
              <View style={{ flex: 1 }}>
                <FieldLabel>Item Code</FieldLabel>
                <TextInput style={s.input} value={form.code} onChangeText={v => setForm(f => ({ ...f, code: v }))} placeholder="SKU / code" placeholderTextColor={COLORS.textMute} />
              </View>
              <View style={{ width: 12 }} />
              <View style={{ flex: 1 }}>
                <FieldLabel>{form.item_type === 'service' ? 'SAC Code' : 'HSN Code'}</FieldLabel>
                <TextInput style={s.input} value={form.hsn} onChangeText={v => setForm(f => ({ ...f, hsn: v }))} placeholder={form.item_type === 'service' ? 'e.g. 9987' : 'e.g. 1006'} placeholderTextColor={COLORS.textMute} keyboardType="numeric" />
              </View>
            </View>
            ...
            <View style={s.row}>
              <View style={{ flex: 1 }}>
                ...
                <TextInput style={s.input} value={form.sale_price} onChangeText={v => setForm(f => ({ ...f, sale_price: v }))} placeholder="0.00" placeholderTextColor={COLORS.textMute} keyboardType="decimal-pad" />
              </View>
              <View style={{ width: 12 }} />
              <View style={{ flex: 1 }}>
                <FieldLabel>{form.item_type === 'service' ? 'Cost / Expense (₹)' : 'Purchase Price (₹)'}</FieldLabel>
                <TextInput style={s.input} value={form.purchase_price} onChangeText={v => setForm(f => ({ ...f, purchase_price: v }))} placeholder="0.00" placeholderTextColor={COLORS.textMute} keyboardType="decimal-pad" />
              </View>
            </View>
            <FieldLabel>GST Rate</FieldLabel>
            <View style={s.gstRow}>
              {GST_RATES.map(r => (
                <TouchableOpacity key={r} style={[s.gstChip, form.gst_rate === r && s.gstChipActive]} onPress={() => setForm(f => ({ ...f, gst_rate: r }))}>
                  <Text style={[s.gstChipText, form.gst_rate === r && s.gstChipTextActive]}>{r}%</Text>
                </TouchableOpacity>
              ))}
            </View>
            {form.item_type === 'product' && (
              <View style={s.row}>
                <View style={{ flex: 1 }}>
                  ...
                  <TextInput style={s.input} value={form.stock} onChangeText={v => setForm(f => ({ ...f, stock: v }))} placeholder="0" placeholderTextColor={COLORS.textMute} keyboardType="decimal-pad" />
                </View>
                <View style={{ width: 12 }} />
                <View style={{ flex: 1 }}>
                  <FieldLabel>Min Stock Alert</FieldLabel>
                  <TextInput style={s.input} value={form.min_stock} onChangeText={v => setForm(f => ({ ...f, min_stock: v }))} placeholder="0" placeholderTextColor={COLORS.textMute} keyboardType="decimal-pad" />
                </View>
              </View>
            )}
            <View style={{ height: 40 }} />
...
function KPI({ label, value, color }) {
  return (<View style={s.kpiChip}><Text style={[s.kpiVal, { color }]}>{value}</Text><Text style={s.kpiLbl}>{label}</Text></View>);
}

function SortHeader({ label, colKey, flex, width, align, sortKey, sortAsc, onSort }) {
  const active = sortKey === colKey;
  return (
    <TouchableOpacity style={[{ paddingHorizontal: 8, paddingVertical: 9 }, flex ? { flex } : { width }]} onPress={() => onSort(colKey)} activeOpacity={0.7}>
      <Text style={[s.thTxt, { textAlign: align }, active && s.thTxtActive]} numberOfLines={1}>{label}{active ? (sortAsc ? ' ↑' : ' ↓') : ''}</Text>
    </TouchableOpacity>
  );
}
