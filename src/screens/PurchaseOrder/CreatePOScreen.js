  import Icon from '../../utils/Icon';
    ...
    export default function CreatePOScreen({ navigation, route }) {
      ...
      const [datePickerTarget, setDatePickerTarget]   = useState(null); // 'date' | 'valid'
      ...
      useEffect(() => {
        (async () => {
          const [p, itms] = await Promise.all([getParties('supplier'), getItems()]);
          setParties(p);
          setItems(itms);
        })();
        if (editPO) {
      ...
      const openDatePicker = (target) => {
        const dateStr = target === 'date' ? date : validUntil;
      ...
      const getDaysInMonth  = (y, m0) => new Date(y, m0 + 1, 0).getDate();
      const getFirstDayOfMonth = (y, m0) => new Date(y, m0, 1).getDay();
      ...
      const confirmItem = () => {
        if (!draftItem.name.trim()) { showAlert('Error', 'Enter item name'); return; }
      ...
      const handleSave = async () => {
        if (!party) { showAlert('Error', 'Select a supplier'); return; }
        const validItems = lineItems.filter(i => i.name.trim() && parseFloat(i.qty_ordered) > 0);
        if (validItems.length === 0) { showAlert('Error', 'Add at least one item'); return; }
        ...
        <View style={[styles.container, { paddingTop: insets.top }]}>
          ...
          <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={styles.scroll}>
            ...
            <TouchableOpacity style={styles.pickerBtn} onPress={() => { setPartySearch(''); setPartyModal(true); }}>
              {party ? (
                ...
                <Text style={styles.pickerPlaceholder}>Select supplier / vendor...</Text>
              )}
            ...
            <View style={styles.row}>
              <View style={{ flex: 1 }}>
                <SL>PO Date *</SL>
                <TouchableOpacity style={styles.dateBtn} onPress={() => openDatePicker('date')}>
                  <Icon name="calendar" size={15} color={COLORS.primary} />
                  <Text style={styles.dateBtnTxt}>{date || 'Select date'}</Text>
                  <Icon name="chevron-down" size={14} color={COLORS.textMute} />
                </TouchableOpacity>
              </View>
              <View style={{ width: 12 }} />
              <View style={{ flex: 1 }}>
                <SL>Valid Until</SL>
                <TouchableOpacity style={styles.dateBtn} onPress={() => openDatePicker('valid')}>
                  <Icon name="calendar" size={15} color={COLORS.primary} />
                  <Text style={styles.dateBtnTxt}>{validUntil || 'Select date'}</Text>
                  <Icon name="chevron-down" size={14} color={COLORS.textMute} />
                </TouchableOpacity>
              </View>
            </View>
            ...
            {lineItems.length === 0 ? (
              <TouchableOpacity style={styles.emptyItems} onPress={openAddItem}>
                ...
                <Text style={styles.emptyItemsTxt}>Tap to add items being ordered</Text>
              </TouchableOpacity>
            ) : (
          ...
          <Modal visible={datePickerVisible} animationType="fade" transparent presentationStyle="overFullScreen">
            <View style={{ flex:1, backgroundColor:'rgba(0,0,0,0.45)', justifyContent:'center', alignItems:'center', padding:20 }}>
              <View style={{ backgroundColor: COLORS.card, borderRadius:18, width:'100%', maxWidth:360, padding:20 }}>

                {/* Header */}
                <View style={{ flexDirection:'row', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
                  <Text style={{ fontSize:16, fontWeight:'700', color:COLORS.text }}>
                    {datePickerTarget === 'date' ? 'PO Date' : 'Valid Until'}
                  </Text>
                  ...
                  <View key={ri} style={{ flexDirection:'row', marginBottom:2 }}>
                    {row.map((day, ci) => {
                      ...
                      return (
                        <TouchableOpacity
                          key={ci}
                          style={{ flex:1, alignItems:'center', justifyContent:'center', paddingVertical:6, borderRadius:20, backgroundColor: isSelected ? COLORS.primary : 'transparent' }}
                          onPress={() => day && selectCalDay(day)}
                          disabled={!day}
                        >
                          <Text style={{ fontSize:13, color: !day ? 'transparent' : isSelected ? '#fff' : COLORS.text, fontWeight: isSelected ? '700' : '400' }}>
                            {day || ''}
                          </Text>
                        </TouchableOpacity>
                      );
            ...
            <View style={styles.modalOverlay}>
              <View style={styles.modalSheet}>
                ...
                <FlatList
                  ...
                  ListEmptyComponent={<Text style={styles.modalEmpty}>No suppliers found</Text>}
                />
              </View>
            </View>
          </Modal>
    ...
    const styles = StyleSheet.create({
      ...
      dateBtn:  { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: COLORS.card, borderWidth: 1, borderColor: COLORS.border, borderRadius: RADIUS.md, paddingHorizontal: 14, paddingVertical: 13 },
      dateBtnTxt: { flex: 1, fontSize: 14, color: COLORS.text, fontWeight: FONTS.medium },
