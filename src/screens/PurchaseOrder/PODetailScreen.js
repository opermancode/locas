  ...
  export default function PODetailScreen({ navigation, route }) {
    ...
    const handleDeliveryConfirm = async ({ poId: pid, deliveries }) => {
      try {
        await recordPODelivery(pid, deliveries);
        setDeliveryModal(false);
        load(); // refresh to show updated progress bars
      } catch (e) {
        if (Platform.OS === 'web') {
          ...
          <View style={s.actionsRow}>
            {po.status !== 'cancelled' && po.status !== 'completed' ? (
              ...
              >
                ...
                <Text style={s.actionSecondaryTxt}>Record Delivery</Text>
              </TouchableOpacity>
            ) : null}
            <TouchableOpacity
  ...
  const s = StyleSheet.create({
    ...
    actionSecondary: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: COLORS.primaryLight, paddingVertical: 14, borderRadius: RADIUS.lg, borderWidth: 1, borderColor: COLORS.primary + '44' },
    actionSecondaryTxt: { color: COLORS.primary, fontWeight: FONTS.bold, fontSize: 14 },
