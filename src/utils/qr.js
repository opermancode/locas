// Pure JS QR Code generator for UPI payments
...
const QRRSBlock = (() => {
  ...
  function getRsBlockTable(typeNumber, errorCorrectLevel) {
    switch (errorCorrectLevel) {
      case QRErrorCorrectLevel.L: return RS_BLOCK_TABLE.slice((typeNumber-1)*12+0, (typeNumber-1)*12+3);
      case QRErrorCorrectLevel.M: return RS_BLOCK_TABLE.slice((typeNumber-1)*12+3, (typeNumber-1)*12+6);
      case QRErrorCorrectLevel.Q: return RS_BLOCK_TABLE.slice((typeNumber-1)*12+6, (typeNumber-1)*12+9);
      case QRErrorCorrectLevel.H: return RS_BLOCK_TABLE.slice((typeNumber-1)*12+9, (typeNumber-1)*12+12);
    }
  }
  return {
...
function getTypeNumberForData(dataLength) {
  // Capacity table for Mode 4 (byte), Error correction M
  const capacity = [
    0,  // placeholder index 0
    16, 28, 44, 64, 86, 108, 124, 154, 182, 216,  // 1-10
    254, 290, 334, 365, 415, 453, 507, 563, 627, 669, // 11-20
    714, 782, 860, 914, 1000, 1062, 1128, 1193, 1267, 1373, // 21-30
    1455, 1541, 1631, 1725, 1812, 1914, 1992, 2102, 2216  // 31-39
  ];
  for (let i = 1; i < capacity.length; i++) {
    if (dataLength <= capacity[i]) return i;
  }
  return 40;
}
...
export function generateUPIQRDataURI(upiId, name = '', amount = 0) {
  try {
    ...
    const typeNumber = getTypeNumberForData(upiURL.length);
