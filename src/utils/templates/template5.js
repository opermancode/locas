...
function line(char = '-', len = 40) { return char.repeat(len); }
...
export default function template5(invoice, profile) {
  ...
  const itemRows = (inv.items || []).map(item => `
    ...
    <div class="item-sub">
      ...
      ${isInter
        ? 'IGST ₹' + r(item.igst)
        : 'CGST ₹' + r(item.cgst) + ' | SGST ₹' + r(item.sgst)
      }
    </div>
...
<div class="inv-meta">
  Date: ${inv.date}
  ${inv.due_date ? '<br>Due: ' + inv.due_date : ''}
  <br>
...
<div style="font-size:10px;margin-top:4px">
  Supply: ${isInter ? 'Inter-state (IGST)' : 'Intra-state (CGST+SGST)'}
</div>
...
<div class="note-section">${inv.notes}</div>` : ''}
