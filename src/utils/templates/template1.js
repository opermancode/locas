  /* ─ helpers ─────────────────────────────────────────────────── */
      ...
      export default function template1(invoice, profile, accent='#1E40AF', upiBlock='') {
        ...
        // Bank details block — always shown if bank info exists
        const bankBlock = hasBank ? `
          <div class="bank">
            ...
            ${prof.branch?'Branch: '+prof.branch+'<br>':''}
            ${prof.account_name?'Name: '+prof.account_name:''}
          </div>` : '';

        // QR block + bank always together in left notes column
        const paymentBlock = hasQR
          ? upiBlock + bankBlock          // QR on top, bank details below QR
          : bankBlock;                    // no QR → just bank details
      ...
      <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .bot { display:flex; gap:10px; page-break-before:always; break-before:always; }
        ...
        .bank { border:1px solid ${accent}22; padding:7px 9px; margin-top:5px; background:${accent}06; border-radius:3px; font-size:9pt; line-height:1.9; }
      ...
      <div class="content">
        ...
        <!-- Meta bar: Invoice No. | Date | PO Number + PO Date (only if PO exists) -->
        <div class="mbar">
          ...
          ${inv.po_number ? `
            <div class="mc"><div class="ml">PO Number</div><div class="mv">${inv.po_number}</div></div>
            <div class="mc"><div class="ml">PO Date</div><div class="mv">${inv.po_date ? fmtDate(inv.po_date) : '—'}</div></div>
          ` : ''}
        </div>
