...
body{font-family:Arial,sans-serif;font-size:11px;color:#111;display:flex;min-height:100vh}
.sidebar{width:8px;background:${accent};flex-shrink:0}
.page{flex:1;padding:22px 26px}
.top{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:16px;padding-bottom:14px;border-bottom:2px solid ${accent}}
.bn{font-size:19px;font-weight:800;color:#111;margin-bottom:3px}
.bd{font-size:10px;color:#555;line-height:1.8}
.il{font-size:28px;font-weight:900;color:${accent};letter-spacing:3px;text-align:right}
.im{text-align:right;font-size:10px;color:#555;line-height:1.8;margin-top:4px}
...
.mbar{display:flex;border:1px solid ${accent}33;margin-bottom:12px;border-radius:3px;overflow:hidden}
.mc{flex:1;padding:6px 10px;border-right:1px solid ${accent}22}
...
.parties{display:flex;border:1px solid ${accent}33;margin-bottom:12px}
.pc{flex:1;padding:10px 12px}
.pc+.pc{border-left:1px solid ${accent}22}
.ph{font-size:9px;font-weight:800;color:${accent};text-transform:uppercase;letter-spacing:1px;padding-bottom:4px;margin-bottom:6px;border-bottom:1px solid ${accent}33}
...
td{padding:7px 6px;font-size:10px;border-bottom:1px solid #F0F0F0;vertical-align:middle}
.tc{text-align:center}.tr{text-align:right}.tl{text-align:left}.b{font-weight:700}.sm{font-size:9px}.xs{font-size:9px;color:#999}
.bot{display:flex;gap:10px;margin-bottom:12px}
.notes{flex:1}
.wbox{background:${accent}08;border:1px solid ${accent}22;padding:7px 10px;border-radius:3px;margin-bottom:6px}
...
.tcbox{background:#f9f9f9;border-left:3px solid ${accent};padding:7px 10px;margin-bottom:6px;font-size:10px;color:#444;line-height:1.7}
.tright{width:238px}
...
.bank{border:1px solid ${accent}22;padding:8px 10px;margin-bottom:6px;background:${accent}06;border-radius:3px;font-size:10px}
.blab{font-size:9px;font-weight:700;color:${accent};text-transform:uppercase;margin-bottom:3px}
.sign{border:1px solid ${accent}22;border-radius:3px;overflow:hidden}
.sh{background:${accent}12;padding:5px 10px;font-size:9px;font-weight:700;color:${accent};text-transform:uppercase;border-bottom:1px solid ${accent}18}
.sb{padding:10px;min-height:62px;display:flex;flex-direction:column;justify-content:space-between}
.seal{width:42px;height:42px;border-radius:50%;border:1.5px dashed ${accent}66;display:flex;align-items:center;justify-content:center;font-size:8px;color:${accent};text-align:center;font-weight:700;margin:0 auto}
.sline{border-top:1px solid #ccc;padding-top:3px;font-size:9px;color:#666;text-align:center;margin-top:8px}
.footer{background:${accent};color:#fff;padding:7px 26px;display:flex;justify-content:space-between;align-items:center;font-size:10px;font-weight:600}
.fs{font-size:9px;opacity:.7}
</style></head><body>
<div class="sidebar"></div>
<div class="page">
<div class="top">
  <div>
    <div class="bn">${prof.name||'My Business'}</div>
    <div class="bd">${prof.address?prof.address+'<br>':''}${prof.phone?'📞 '+prof.phone+(prof.email?' &nbsp;|&nbsp; '+prof.email:'')+'<br>':''}${prof.gstin?'GSTIN: <b>'+prof.gstin+'</b>':''}${prof.pan?' &nbsp;|&nbsp; PAN: '+prof.pan:''}</div>
  </div>
  <div>
    <div class="il">TAX INVOICE</div>
    <div class="im"><span class="in">${inv.invoice_number}</span><br>Date: ${inv.date}${inv.due_date?'<br>Due: '+inv.due_date:''}<br><span class="badge ${inv.status||'unpaid'}">${(inv.status||'UNPAID').toUpperCase()}</span></div>
  </div>
</div>
<div class="mbar">
...
<div class="parties">
  <div class="pc"><div class="ph">Bill To</div><div class="pn">${inv.party_name||'Walk-in Customer'}</div><div class="pd">${inv.party_address?inv.party_address+'<br>':''}${inv.party_gstin?'GSTIN: '+inv.party_gstin+'<br>':''}${inv.party_state?'State: '+inv.party_state+' ('+(inv.party_state_code||'')+')':''}</div><span class="stag">${isInter?'🔀 IGST':'✅ CGST+SGST'}</span></div>
  <div class="pc"><div class="ph">Seller</div><div class="pn">${prof.name||'My Business'}</div><div class="pd">${prof.address?prof.address+'<br>':''}${prof.gstin?'GSTIN: '+prof.gstin+'<br>':''}${prof.state?'State: '+prof.state+' ('+(prof.state_code||'')+')':''}</div></div>
</div>
<table>
  <thead><tr>
    <th class="tc" style="width:4%">#</th><th style="width:24%">Item / Description</th>
...
<div class="bot">
  ...
  <div class="tright">
    ...
    ${prof.bank_name||prof.account_no?`<div class="bank" style="margin-top:6px"><div class="blab">Bank Details</div>${prof.bank_name?'<b>'+prof.bank_name+'</b><br>':''}${prof.account_no?'A/C: '+prof.account_no+'<br>':''}${prof.ifsc?'IFSC: '+prof.ifsc:''}</div>`:''}
