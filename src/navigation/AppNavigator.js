      ...
      const renderNavItem = (item, idx) => {
        ...
        return (
          ...
          >
            ...
            {/* Flyout popup — appears to the right of sidebar, works in both expanded and collapsed */}
            {isFlying && hasFlyout && (
              <div
                style={{
                  position: 'fixed',
                ...
                onMouseLeave={() => setFlyoutItem(null)}
              >
                <div style={{ padding:'4px 10px 8px', fontSize:10, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.1em', color:'rgba(255,255,255,0.25)', borderBottom:'1px solid rgba(255,255,255,0.06)', marginBottom:4 }}>
                  {item.label}
                </div>
                {flyouts.map((f, fi) => (
                  <div
                    ...
                    style={{
                      ...
                      backgroundColor: f.primary ? 'rgba(255,107,0,0.1)' : 'transparent',
                      transition:'background-color 0.12s',
                      marginBottom: fi < flyouts.length - 1 ? 2 : 0,
                    }}
                    onMouseEnter={e => e.currentTarget.style.backgroundColor = f.primary ? 'rgba(255,107,0,0.22)' : 'rgba(255,255,255,0.07)'}
                    onMouseLeave={e => e.currentTarget.style.backgroundColor = f.primary ? 'rgba(255,107,0,0.1)' : 'transparent'}
                  >
                    <div style={{ width:26, height:26, borderRadius:6, backgroundColor: f.primary ? 'rgba(255,107,0,0.15)' : 'rgba(255,255,255,0.06)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                      <SvgIcon name={f.icon} size={13} color={f.primary ? '#FF6B00' : 'rgba(255,255,255,0.55)'} strokeWidth={2} />
                    </div>
                    <span style={{ fontSize:13, color: f.primary ? '#FF7A24' : 'rgba(255,255,255,0.8)', fontWeight: f.primary ? 600 : 400 }}>
                      {f.label}
                    </span>
                  </div>
                ))}
