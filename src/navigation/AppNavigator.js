  ...
  const SIDEBAR_FLYOUTS = {
    ...
    PurchaseOrders: [
      ...
      { label:'Alerts',    icon:'alert-triangle', tab:'More', screen:'POAlerts' },
      { label:'Backlog',   icon:'shopping-bag',   tab:'More', screen:'POBacklog' },
    ],
    QuotationsTab: [
  ...
  // ── Dropdown config — inline expanding menus ──────────────────────
  const SIDEBAR_DROPDOWNS = {
    Reports: [
      { label:'Overview',   tab:'More', screen:'Reports', params:{ tab:'overview'  } },
      { label:'Income',     tab:'More', screen:'Reports', params:{ tab:'income'    } },
      { label:'Sales',      tab:'More', screen:'Reports', params:{ tab:'sales'     } },
      { label:'Purchases',  tab:'More', screen:'Reports', params:{ tab:'purchases' } },
      { label:'Expenses',   tab:'More', screen:'Reports', params:{ tab:'expenses'  } },
      { label:'GST / GSTR', tab:'More', screen:'Reports', params:{ tab:'gst'       } },
      { label:'P & L',      tab:'More', screen:'Reports', params:{ tab:'pl'        } },
    ],
    PurchaseOrders: [
      { label:'All PO Orders', tab:'More', screen:'PurchaseOrders', primary:true },
      { label:'New PO',        tab:'More', screen:'CreatePO',  primary:true },
      { label:'Alerts',        tab:'More', screen:'POAlerts' },
      { label:'Backlog',       tab:'More', screen:'POBacklog' },
    ],
  };
  ...
  function DesktopSidebar({ state, navigation, onWidthChange }) {
    ...
    React.useEffect(()=>{ if (!expanded) setOpenDD(null); }, [expanded]);
    ...
    const renderItem = (item) => {
      ...
      const hasDD      = !!SIDEBAR_DROPDOWNS[item.name];
      const ddOpen     = openDD === item.name;

      const handleClick = () => {
        if (hasDD && expanded) {
          setOpenDD(ddOpen ? null : item.name);
        } else {
          setOpenDD(null);
          go(item);
        }
        ...
        <div key={item.name}>
          ...
          <div
            ...
            style={{
              ...
              backgroundColor: active ? ITEM_ACTIVE : ddOpen ? 'rgba(255,255,255,0.07)' : 'transparent',
              transition:'background 0.12s',
            }}
            onMouseEnter={e=>{ if(!active&&!ddOpen) e.currentTarget.style.backgroundColor=ITEM_HOVER; }}
            onMouseLeave={e=>{ if(!active) e.currentTarget.style.backgroundColor=ddOpen?'rgba(255,255,255,0.07)':'transparent'; }}
          >
            {/* Active bar */}
            {active&&<div style={{ position:'absolute', left:0, top:6, bottom:6, width:3, borderRadius:2, backgroundColor:BRAND }}/>}

            {/* Icon */}
            <div style={{ width:22, height:22, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
              <SvgIcon name={item.icon} size={16}
                color={active?BRAND:ddOpen?'rgba(255,255,255,0.7)':'rgba(255,255,255,0.35)'}
            ...
            {/* Label */}
            <span style={{
              fontSize:13, fontWeight:active||ddOpen?'700':'400',
              color:active?'#fff':ddOpen?'rgba(255,255,255,0.8)':'rgba(255,255,255,0.4)',
            ...
            {/* Dropdown chevron */}
            {expanded&&hasDD&&(
              <div style={{ flexShrink:0, transition:'transform 0.2s', transform:ddOpen?'rotate(90deg)':'rotate(0deg)', opacity:ddOpen?0.7:0.35 }}>
                <SvgIcon name="chevron-right" size={11} color="rgba(255,255,255,0.5)" strokeWidth={2}/>
              </div>
            )}
          </div>

          {/* Inline dropdown — only when expanded */}
          {hasDD && expanded && ddOpen && (
            <div style={{
              margin:'2px 6px 4px 16px',
              borderLeft:'2px solid rgba(255,255,255,0.08)',
              paddingLeft:8,
            }}>
              {SIDEBAR_DROPDOWNS[item.name].map((sub, si) => {
                const subActive = activeTabName==='More' && activeMoreScreen===sub.screen;
                return (
                  <div
                    key={si}
                    onClick={()=>go(sub)}
                    style={{
                      ...
                      cursor:'pointer',
                      fontSize:12,
                      fontWeight:sub.primary?'600':'400',
                      color: subActive ? BRAND : sub.primary ? 'rgba(255,255,255,0.8)' : 'rgba(255,255,255,0.45)',
                      backgroundColor: subActive ? ITEM_ACTIVE : 'transparent',
                      transition:'background 0.1s, color 0.1s',
                      whiteSpace:'nowrap',
                      borderLeft: subActive ? `2px solid ${BRAND}` : '2px solid transparent',
                      marginBottom:1,
                    }}
                    onMouseEnter={e=>{ if(!subActive){ e.currentTarget.style.backgroundColor=ITEM_HOVER; e.currentTarget.style.color='#fff'; } }}
                    onMouseLeave={e=>{ if(!subActive){ e.currentTarget.style.backgroundColor=subActive?ITEM_ACTIVE:'transparent'; e.currentTarget.style.color=sub.primary?'rgba(255,255,255,0.8)':'rgba(255,255,255,0.45)'; } }}
                  >
                    {sub.label}
                  </div>
                );
    ...
    return (
      ...
      >
        ...
        <div style={{ padding:'0 10px', borderBottom:`1px solid ${BORDER}`, display:'flex', alignItems:'center', height:56, gap:10, overflow:'hidden' }}>
          ...
          {expanded&&(
            <div onClick={()=>setPinned(v=>!v)} style={{ cursor:'pointer', marginLeft:'auto', opacity:0.3, flexShrink:0 }}
