      ...
      const renderNavItem = (item, idx) => {
        ...
        return (
          <div
            ...
            onMouseLeave={() => {
              if (!expanded) setFlyoutItem(null);
            }}
          >
            <div
              onClick={() => go(item)}
              style={{
                ...
                backgroundColor: active ? 'rgba(255,255,255,0.08)' : isFlying ? 'rgba(255,255,255,0.06)' : 'transparent',
                transition: 'background-color 0.15s', overflow: 'hidden',
              }}
              onMouseEnter={e => { if (!active && expanded) e.currentTarget.style.backgroundColor = 'rgba(255,255,255,0.04)'; }}
              onMouseLeave={e => { if (!active && expanded) e.currentTarget.style.backgroundColor = 'transparent'; }}
            >
              {active && (
            ...
            {/* Flyout menu — only when collapsed + hovering */}
            {!expanded && isFlying && hasFlyout && (
              <div
                style={{
                  position: 'fixed',
                  left: W_COLLAPSED + 6,
              ...
              >
                {/* Section label */}
                <div style={{ padding:'4px 8px 6px', fontSize:10, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.08em', color:'rgba(255,255,255,0.3)' }}>
                  {item.label}
                </div>
                {flyouts.map((f, fi) => (
                  <div
                    ...
                    style={{
                      ...
                      backgroundColor: f.primary ? 'rgba(255,107,0,0.12)' : 'transparent',
                      transition:'background-color 0.12s',
                    }}
                    onMouseEnter={e => e.currentTarget.style.backgroundColor = f.primary ? 'rgba(255,107,0,0.2)' : 'rgba(255,255,255,0.06)'}
                    onMouseLeave={e => e.currentTarget.style.backgroundColor = f.primary ? 'rgba(255,107,0,0.12)' : 'transparent'}
                  >
                    <SvgIcon name={f.icon} size={13} color={f.primary ? '#FF6B00' : 'rgba(255,255,255,0.6)'} strokeWidth={2} />
                    <span style={{ fontSize:13, color: f.primary ? '#FF6B00' : 'rgba(255,255,255,0.8)', fontWeight: f.primary ? 600 : 400 }}>
                      {f.label}
                    </span>
                  </div>
                ))}
