...
function DesktopSidebar({ state, navigation, onWidthChange }) {
  ...
  const openFlyout = (name, y) => {
    if (flyoutTimer.current) clearTimeout(flyoutTimer.current);
    setFlyoutY(y);
    setFlyoutItem(name);
  };
  const closeFlyout = () => {
    flyoutTimer.current = setTimeout(() => setFlyoutItem(null), 120);
  };
  const keepFlyout = (name) => {
    if (flyoutTimer.current) clearTimeout(flyoutTimer.current);
    setFlyoutItem(name);
  };
  ...
  const renderItem = (item, isFooter=false) => {
    ...
    return (
      <div
        ...
        onMouseLeave={() => { if (hasFlyout) closeFlyout(); else setFlyoutItem(null); }}
      >
        <div
          onClick={() => go(item)}
          style={{
  ...
  return (
    <div
      ...
      onMouseLeave={()=>{ setHovered(false); closeFlyout(); }}
    >
      ...
      {flyoutItem&&(()=>{
        ...
        return (
          <div
            ...
            onMouseEnter={()=>keepFlyout(currentItem)}
